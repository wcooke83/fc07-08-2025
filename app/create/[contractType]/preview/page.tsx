...
const replacePlaceholders = (template: string, data: Record<string, string>): string => {
  ...
  // Create a map to track which field keys we've already processed
  const processedFields = new Set<string>()
  
  // Define placeholder patterns in order of priority (most specific first)
  ...
  Object.entries(data).forEach(([key, value]) => {
    // Skip if value is empty or undefined, or if we've already processed this field
    if (!value || value.trim() === '' || processedFields.has(key)) return
    
    // Try each placeholder pattern and replace ALL occurrences of any pattern that exists
    for (const patternFn of placeholderPatterns) {
      const placeholder = patternFn(key)
      
      // Replace ALL occurrences of this placeholder if it exists in the content
      if (content.includes(placeholder)) {
        content = content.replace(new RegExp(escapeRegExp(placeholder), 'g'), value)
        // Don't break here - we want to check other patterns too
      }
    }
    
    // Mark this field as processed so we don't process it again
    ...
    try {
      ...
      if (savedData) {
        ...
        if (templateError) {
          ...
          // Use the improved placeholder replacement function
          const content = replacePlaceholders(template.template_content, parsedData)
...
WHEREAS, the Disclosing Party possesses certain confidential and proprietary information that it wishes to disclose to the Receiving Party for the purpose of {{purpose}};
...
Date: _______________               Date: _______________`
  ...
  // Apply the same improved replacement logic to fallback template
  return replacePlaceholders(template, parsedData)
}
          ...
          <Card className="rounded-xl border bg-card text-card-foreground shadow-lg">
            ...
            <CardContent className="p-6">
              ...
              <div className="flex justify-between mt-8">
                <Button variant="outline" onClick={handleBack} className="h-11 px-8">
